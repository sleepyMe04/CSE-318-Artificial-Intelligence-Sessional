...
TreeNode* buildTree(const vector<Example>& examples, const vector<string>& attributes,
                    const string& criterion, double alpha, int depth, int& forcedLeaves) {
    TreeNode* node = new TreeNode();

    // Base cases 
    if (examples.empty() || isPure(examples) || attributes.empty()) {
        node->isLeaf = true;
    ...
    // Stochastic pruning
    double stopProb = min(1.0, alpha * depth);
    uniform_real_distribution<double> dist(0.0, 1.0);
    random_device rd;
    mt19937 gen(rd());
    
    if (dist(gen) < stopProb) {
        node->isLeaf = true;
        node->label = getMajorityClass(examples);
        cout<<"Node pruned at depth "<< getTreeDepth(node)<<endl;
        forcedLeaves++;
        return node;
    }

    // Feature bagging
    int k = floor(sqrt(attributes.size()));
    vector<string> randomFeatures = getRandomFeatures(attributes, k);

    string bestAttr = chooseBestAttribute(examples, randomFeatures, criterion);
    if (bestAttr.empty()) {
        node->isLeaf = true;
        node->label = getMajorityClass(examples);
        return node;
    ...
    for (const string& attr : attributes) {
        if (attr != bestAttr) remaining.push_back(attr);
    }

    auto splits = splitData(examples, bestAttr);
    for (const auto& pair : splits) {
        if (!pair.second.empty()) {
            node->children[pair.first] = buildTree(pair.second, remaining, criterion, alpha, depth + 1, forcedLeaves);
        }
    }
...
vector<string> getRandomFeatures(const vector<string>& atrributes ,int k)
{
    int i=0;
    vector <string> shuffled= atrributes;
     mt19937 rng(chrono::high_resolution_clock::now().time_since_epoch().count() + i++);
        ...
        return vector<string>(shuffled.begin(), shuffled.begin() + k);
...
// vector<string> getRandomFeatures(const vector<string>& attributes, int k) {
//     vector<string> shuffled = attributes;
//     random_shuffle(shuffled.begin(), shuffled.end());
//     if (k > shuffled.size()) k = shuffled.size();
//     return vector<string>(shuffled.begin(), shuffled.begin() + k);

int main(int argc, char* argv[]) {
    if (argc != 5) {
        cerr << "Usage: ./decision_tree <criterion> <maxDepth> <alpha> --dataset <dataset_name>\n";
        return 1;
    }
